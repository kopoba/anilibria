...
var CaptchaCallback = function () {
    recaptcha1 = grecaptcha.render('RecaptchaField', {'sitekey': $('meta[name=recaptcha2_site_key]').attr("content")});
};
...
$(document).ready(function () {
    ...
    ($('[data-type=player]').get() || []).some(player => {
        if ($(player).is(":visible")) {
            tabSwitch($(player).data('tab'));
            return true;
        }
...
$(document).on('click', '[data-light]', function (e) {
    ...
    if ($('.light-off').length > 0) {
        if ($('.light-off').is(":hidden")) {
            ...
            $(".light-off").fadeTo(500, 0, function () {
                $('.light-off').hide();
            });
...
$(document).on('click', '[data-release-delete]', function (e) {
    ...
    if (window.confirm('Действительно хотите удалить релиз?')) {
        $.post("/public/release/delete.php", {
            'id': $('input[id=releaseID]').val(),
            'csrf_token': csrf_token
        }, function (json) {
            data = JSON.parse(json);
