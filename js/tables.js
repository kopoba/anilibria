...
$(document).on("click", "[data-session-id]", function(e) {
	$(this).blur();
	e.preventDefault();
	$.post("/public/close.php", {'id': $(this).data("session-id"), 'csrf_token': csrf_token });
	$(this).hide();
...
$(document).on('click', '[data-admin-release-delete]', function(e){
	...
	if(window.confirm('Действительно хотите удалить релиз?')){
		$.post("/public/release/delete.php", {'id': $(this).data("admin-release-delete"), 'csrf_token': csrf_token});
